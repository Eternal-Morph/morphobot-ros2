...
class GroundTeleop(Node):
    ...
    def compute_controls(self):
        ...
        if is_forward_active and is_yaw_active:
            # Viraj alırken (W+D / W+A): Savrulmayı önlemek için sakin hız ve yumuşak viraj
            base_speed = 1.4 if self.forward_fast else 0.95
            base_yaw = 0.9 if self.yaw_fast else 0.55
            desired_vx = self.forward_command_dir * base_speed * scale
            desired_wz = self.yaw_command_dir * base_yaw * scale
        elif is_forward_active:
            # Sadece düz sürüş (W / S)
            base_speed = 1.8 if self.forward_fast else 1.2
            desired_vx = self.forward_command_dir * base_speed * scale
            desired_wz = 0.0
            self.yaw_command_dir = 0.0
        elif is_yaw_active:
            # Sadece yerinde dönüş (A / D): Sakin ve kontrollü dönüş
            base_yaw = 1.2 if self.yaw_fast else 0.75
            desired_vx = 0.0
            self.target_linear_x = 0.0
            desired_wz = self.yaw_command_dir * base_yaw * scale
            self.forward_command_dir = 0.0
        ...
        smoothing_lin = min(1.0, 8.0 * dt)
        smoothing_ang = min(1.0, 6.0 * dt)
        ...
        # Tekerleklerin eklem eksenine göre linear.x ve angular.z yönleri:
        twist = Twist()
