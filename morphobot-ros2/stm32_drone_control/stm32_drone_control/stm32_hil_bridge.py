#!/usr/bin/env python3
# ==============================================================================
# STM32 HIL Simulation Bridge Node
# Package: stm32_drone_control
# Author: DeepMind & Emirhan
# ==============================================================================
...
class STM32HILBridgeNode(Node):
    """
    Hardware-In-The-Loop (HIL) Bridge Node connecting STM32 Microcontroller,
    ROS 2 / Gazebo Harmonic Simulation, and Pygame Gamepad Controller.
    """
    def __init__(self):
        super().__init__('stm32_hil_bridge_node')
        ...
        # IMU Unit and Inversion Parameters
        ...
        # Motor Mapping (STM32 [M1, M2, M3, M4] -> Gazebo [FR, RL, FL, RR])
        ...
        # 3. Gamepad Initialization
        pygame.init()
        pygame.joystick.init()
        self.joystick = None
        if pygame.joystick.get_count() == 0:
            self.get_logger().warn("⚠️ Gamepad not found! Connect a gamepad/joystick.")
        else:
        ...
        try:
            ...
            self.get_logger().info(f"✅ Serial Port Opened: {self.serial_port_name} @ {self.baud_rate}")
        except Exception as e:
            self.get_logger().error(f"❌ Serial Port Error ({self.serial_port_name}): {e}")

        # 5. ROS 2 Subscriptions & Publishers
        ...
        self.get_logger().info(f"🚀 STM32 HIL Bridge Node Running (50 Hz)... Motor Mapping: {self.motor_indices}")
                ...
                while len(self.rx_buffer) >= 29:
                    if self.rx_buffer[0] == 0x55 and self.rx_buffer[1] == 0xAA:
                        ...
                        if (sum(payload_rx) & 0xFF) == pkt_checksum:
                            ...
                            # Motor Mapping & Publish to Gazebo Harmonic
